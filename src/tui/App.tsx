...
export function App({ cfg }: { cfg: CrewConfig }) {
  ...
  useEffect(() => {
    const tail = new LogTail(snap?.logPath ?? cfg.configDir);
    let cancelled = false;
    ...
    // Poll loop owns its own LogTail per logPath; only the initial mount wires it up.
  ...
  const reserved = 4 + snap.agentRows.length + 2 + (snap.stuck.length ? snap.stuck.length + 2 : 0) + 2;
  const logHeight = Math.max(4, rows - reserved);

  return (
    <Box flexDirection="column">
      <Header snap={snap} />
      <AgentList rows={snap.agentRows} />
      <Box marginTop={1}>
