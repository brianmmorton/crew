...
(what counts as "material"), `personas/*.md` (the agent prompts), and
`AGENTS.md` — a reference doc covering what crew is and every `config.yaml`
field, regenerated by `crew init`/`setup`/`doctor` whenever it's missing or
older than the installed crew version (it's crew's own documentation, not
project config — don't hand-edit it). Highlights of `config.yaml`:
